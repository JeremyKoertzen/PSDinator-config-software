...
Background = np.loadtxt("Background.txt", skiprows=1)
Background_bin_centers = Background[:, 0]
Background_counts = Background[:, 1]
Background_bin_size = Background_bin_centers[1] - Background_bin_centers[0]
...
fig, (Cs137, Mn54, Na22, Co60, Background) = plt.subplots(5, 1, figsize=(10, 12), sharex=True)

Cs137.plot(Cs137_bin_centers, Cs137_counts, label='Cs137', color='orange')
#Cs137.plot(Cs137_x, Cs137_y_fit, 'r--', label='Gaussian Fit')
#Cs137.plot(Cs137_x_compton, Cs137_y_compton, 'g-', label=f'Compton Edge: {Cs137_compton_edge:.2f} ± {Cs137_x0_err:.2f}')
...
#Mn54.plot(Mn54_x_compton, Mn54_y_compton, 'g-', label=f'Compton Edge: {Mn54_compton_edge:.2f} ± {Mn54_x0_err:.2f}')
Mn54.set_yscale('log')
...
#Na22.plot(Na22_x_compton, Na22_y_compton, 'g-', label=f'Compton Edge: {Na22_compton_edge:.2f} ± {Na22_x0_err:.2f}')
Na22.set_yscale('log')
...
#Co60.plot(Co60_x_compton, Co60_y_compton, 'g-', label=f'Compton Edge: {Co60_compton_edge:.2f} ± {Co60_x0_err:.2f}')
Co60.set_yscale('log')
...
Background.plot(Background_bin_centers, Background_counts, label='Background', color='purple')
Background.set_yscale('log')
Background.set_xlabel('ADC Value')
Background.set_ylabel('Counts')
Background.set_title('Background Data (Log Scale)')
Background.legend()
Background.grid()
